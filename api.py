...
async def get_cockpit(vin: str, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
...
async def get_location(vin: str, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
...
async def hvac_start(vin: str, temp: float = 21.0, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
...
async def hvac_stop(vin: str, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
...
async def charge_start(vin: str, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
...
async def charge_stop(vin: str, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
...
async def lights(vin: str, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
...
async def honk(vin: str, x_renault_email: str = Header(...), x_renault_password: str = Header(...)):
    try:
        async with aiohttp.ClientSession() as session:
            client = MyRenaultClient(email=x_renault_email, password=x_renault_password, websession=session)
